...
class PAR(nn.Module):
    ...
    def __init__(self, dilations, num_iter, in_channels=512, out_channels=512):
        super().__init__()
    ...
    def get_dilated(self, x):
        b, c, h, w = x.shape
        x_aff = []
        for dilation, dilated_conv in zip(self.dilations, self.dilated_convs):
            _x = dilated_conv(x)
            x_aff.append(_x)
        return torch.cat(x_aff, dim=1)
    ...
    def forward(self, imgs, masks):
        ...
        #print(f'imgs：{imgs.shape}')#[1, 3, 128, 128]
        b, c, h, w = imgs.shape
        _imgs = self.get_dilated_neighbors(imgs)
        #print(f'_imgs：{_imgs.shape}')#[1, 3, 32, 128, 128]
